...
- `pages/2_Chatbot`: Utilizes Hugging Face API key for summaries of reports and relevant natural disaster information through tool calling of APIs such as NASA, FEMA, NWS, and DuckDuckGo Search
...
```bash
git clone https://github.com/DomPTech/Presidential-AI-Challenge-25-26.git
cd Presidential-AI-Challenge-25-26
cp .streamlit/secrets.toml.example .streamlit/secrets.toml
pip install -r requirements.txt

```
*(The `cp` command copies the example secrets file so that you can conveniently paste in your own keys.)*
- Then, plug in our Supabase URL, API key, and Hugging Face API Key into `secrets.toml`.
### Running the App
